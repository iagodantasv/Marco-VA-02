# 🌎 Educação Financeira e Ativos Internacionais

Projeto desenvolvido com foco em **letramento financeiro para o público jovem**, abordando conceitos de organização financeira, investimentos básicos e introdução a ativos internacionais.

O objetivo é criar uma plataforma educativa moderna, acessível e responsiva, utilizando boas práticas de desenvolvimento Front-end e estruturação semântica em HTML.

---

# Sobre o Projeto

Este projeto foi idealizado em parceria com os cursos de:

- Administração
- Ciências Contábeis

A proposta é apresentar conteúdos financeiros de forma simples e visual, ajudando jovens a:

- Organizar suas finanças pessoais
- Entender investimentos básicos
- Conhecer ativos internacionais
- Aprender sobre rendimentos atrelados ao dólar
- Comparar diferentes tipos de investimentos

---

# Objetivos do Projeto

- Promover educação financeira acessível
- Desenvolver páginas responsivas e organizadas
- Aplicar boas práticas de HTML semântico
- Trabalhar tabelas comparativas responsivas
- Melhorar a experiência visual do usuário
- Criar um projeto moderno para portfólio acadêmico

---

# Tecnologias Utilizadas

- HTML5
- CSS3
- JavaScript
- Git
- GitHub

---

# Funcionalidades

## Comparação de Investimentos

Tabelas responsivas com comparação entre:

- CDB
- Tesouro Direto
- Conta remunerada
- Dólar
- ETFs internacionais

---

## Planejamento Financeiro

- Organização de gastos
- Controle financeiro
- Metas financeiras
- Reserva de emergência

---

## Ativos Internacionais

Conteúdo introdutório sobre:

- Investimentos dolarizados
- ETFs
- Ações internacionais
- Variação cambial

---

# Responsividade

O projeto foi desenvolvido com foco em:

- Desktop
- Tablets
- Smartphones

Garantindo boa visualização em diferentes tamanhos de tela.







# Colaboradores

| Nome | Função |
|------|---------|
| coloque seu nome aqui | função aqui |
| coloque seu nome aqui | função aqui |
| Renato Firmino Santos da Silva | Desenvolvedor |
| coloque seu nome aqui | função aqui |
| coloque seu nome aqui | função aqui |

---

# Desafios Front-end

Durante o desenvolvimento, os principais desafios foram:

- Construção de tabelas responsivas
- Organização semântica de dados financeiros
- Criação de layout moderno e intuitivo
- Estruturação de componentes reutilizáveis
- Adaptação para diferentes dispositivos

---

# Aprendizados

Este projeto permitiu desenvolver habilidades em:

- HTML semântico
- CSS responsivo
- Organização de projetos Front-end
- Trabalho em equipe com Git/GitHub
- Estruturação de conteúdo financeiro

---

# 📬 Contato

## Renato Firmino Santos da Silva
- GitHub: https://github.com/Renatofsds16
- Email: renatofss16@gmail.com

## seu nome aqui
- GitHub: https://github.com/seuusuario
- Email: email@gmail.com


## seu nome aqui
- GitHub: https://github.com/seuusuario
- Email: email@gmail.com


## seu nome aqui
- GitHub: https://github.com/seuusuario
- Email: email@gmail.com


## seu nome aqui
- GitHub: https://github.com/seuusuario
- Email: email@gmail.com
---
